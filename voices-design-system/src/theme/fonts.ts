/**
 * The display face, self-hosted (SIL OFL, licence beside the file) and
 * resolved against THIS module's origin so a federated consumer never
 * fetches it from a path that only exists in its own bundle. The body face
 * is Helvetica Neue from the system.
 *
...
export const FONT_FACE_CSS = `@font-face{font-family:"Anton";font-style:normal;font-weight:400;font-display:swap;src:url("${ANTON}") format("woff2");}`;
