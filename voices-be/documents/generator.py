...
def pending_runs():
    return GenerationRun.objects.filter(status="pending").select_related("week", "arm").order_by("created_at")


def execute_pending(deps: Dependencies | None = None) -> list[GenerationRun]:
    """Every pending run, oldest first, on the live model and checkpointer."""
